...
export const useAvailability = () => {
  ...
  const checkSpecialtyAvailability = useCallback(async (specialty: Specialty): Promise<SystemAvailability> => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`${import.meta.env.VITE_API_URL}/availability/specialty/${specialty}`, {
        headers: {
  ...
  const getAllSpecialtiesAvailability = useCallback(async (): Promise<SystemAvailability[]> => {
    try {
      setLoading(true);
      const token = localStorage.getItem('token');
      const response = await fetch(`${import.meta.env.VITE_API_URL}/availability/specialties`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
        },
  ...
  const getProfessionalAvailability = useCallback(async (): Promise<ProfessionalAvailability | null> => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`${import.meta.env.VITE_API_URL}/availability/professional`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
        },
  ...
  const updateProfessionalAvailability = useCallback(async (
    isOnline: boolean,
  ...
  const setSpecialty24hAvailability = useCallback(async (
    specialty: Specialty,
    enabled: boolean
  ): Promise<void> => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`${import.meta.env.VITE_API_URL}/availability/specialty/${specialty}/24h`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
  ...
  const getNextAvailableTime = useCallback(async (specialty: Specialty): Promise<{
    nextAvailableTime: string | null;
    isAvailableNow: boolean;
  }> => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`${import.meta.env.VITE_API_URL}/availability/specialty/${specialty}/next-available`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
        },
  ...
  const formatNextAvailableTime = useCallback((nextAvailableTime: string | undefined): string => {
  ...
  const getAvailabilityStatusInfo = useCallback((availability: SystemAvailability): {
  ...
  const getProfessionalStatusInfo = useCallback((status: string): {
    label: string;
    color: string;
    icon: string;
  } => {
