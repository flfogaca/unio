...
export const useWaitTime = () => {
  ...
  const getSpecialtyWaitTime = useCallback(
    async (specialty: Specialty): Promise<WaitTimeData> => {
      try {
        const token = localStorage.getItem('token');
        const response = await fetch(
          `${import.meta.env.VITE_API_URL}/wait-time/specialty/${specialty}`,
          {
            headers: {
  ...
  const getSpecialtiesWaitTimes = useCallback(
    async (specialties: Specialty[]): Promise<WaitTimeData[]> => {
      try {
        setLoading(true);
        const token = localStorage.getItem('token');
        ...
        const response = await fetch(
          `${import.meta.env.VITE_API_URL}/wait-time/specialties?specialties=${specialtiesParam}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
              'Content-Type': 'application/json',
  ...
  const getConsultationWaitTime = useCallback(
    async (consultationId: string): Promise<ConsultationWaitTime> => {
      try {
        const token = localStorage.getItem('token');
        const response = await fetch(
          `${import.meta.env.VITE_API_URL}/wait-time/consultation/${consultationId}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
              'Content-Type': 'application/json',
  ...
  const getQueueStatistics = useCallback(
    async (specialty: Specialty): Promise<QueueStatistics> => {
      try {
        const token = localStorage.getItem('token');
        const response = await fetch(
          `${import.meta.env.VITE_API_URL}/wait-time/statistics/${specialty}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
              'Content-Type': 'application/json',
  ...
  const getHistoricalData = useCallback(
    async (specialty: Specialty, days: number = 7): Promise<HistoricalData> => {
      try {
        const token = localStorage.getItem('token');
        const response = await fetch(
          `${import.meta.env.VITE_API_URL}/wait-time/historical/${specialty}?days=${days}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
              'Content-Type': 'application/json',
  ...
  const getWaitTimeStatus = useCallback(
    (
      queueLength: number,
      onlineProfessionals: number
    ): {
  ...
  const startAutoRefresh = useCallback(
    (specialties: Specialty[], intervalMs: number = 30000) => {
