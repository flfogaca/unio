...
export function AdminDashboard() {
  ...
  const consultasFinalizadas = items.filter(
    item => item.status === 'finalizado'
  ).length;
  const consultasEmAndamento = items.filter(
    item => item.status === 'em-atendimento'
  ).length;
  const consultasNaFila = items.filter(
    item => item.status === 'em-fila'
  ).length;
  const consultasUrgentes = items.filter(
            ...
            <Card key={stat.title}>
              <CardContent className='p-6'>
                <div className='flex items-center justify-between'>
                  ...
                  <div
                    className={`w-12 h-12 ${stat.color} rounded-lg flex items-center justify-center ml-4`}
                  >
            ...
            <div className='space-y-2'>
              <button className='w-full p-3 text-left border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors'>
                ...
                <div className='text-xs text-gray-500'>
                  Adicionar/remover pacientes e dentistas
                </div>
              </button>
              <button className='w-full p-3 text-left border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors'>
                <div className='font-medium text-sm'>Configurar Fila</div>
                <div className='text-xs text-gray-500'>
                  Ajustar regras e prioridades
                </div>
              </button>
              <button className='w-full p-3 text-left border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors'>
