...
export const AvailabilityDashboard: React.FC<AvailabilityDashboardProps> = ({
  ...
  const handleStatusChange = async (status: 'available' | 'busy' | 'away' | 'offline') => {
    try {
      ...
      <Card className="p-6">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-gray-900">Disponibilidade 24h</h2>
        ...
        <Card className="p-6">
          ...
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-medium">{getSpecialtyName(professionalAvailability.specialty)}</p>
                <p className="text-sm text-gray-500">
                  Última atividade: {new Date(professionalAvailability.lastActivity).toLocaleString('pt-BR')}
                </p>
              </div>
            </div>
            
            <StatusBadge 
              className={getProfessionalStatusInfo(professionalAvailability.currentStatus).color}
            >
              {getProfessionalStatusInfo(professionalAvailability.currentStatus).label}
            </StatusBadge>
          </div>
          ...
          <div className="flex gap-2">
            <Button
              ...
              variant={professionalAvailability.currentStatus === 'available' ? 'default' : 'outline'}
              size="sm"
            ...
            <Button
              ...
              variant={professionalAvailability.currentStatus === 'busy' ? 'default' : 'outline'}
              size="sm"
            ...
            <Button
              ...
              variant={professionalAvailability.currentStatus === 'away' ? 'default' : 'outline'}
              size="sm"
            ...
            <Button
              ...
              variant={professionalAvailability.currentStatus === 'offline' ? 'default' : 'outline'}
              size="sm"
        ...
        <Card className="p-6">
          ...
          <div className="space-y-4">
            <p className="text-sm text-gray-600">
              Configure quais especialidades ficam disponíveis 24 horas por dia, 7 dias por semana.
            </p>
            
            {availabilities.map((availability) => (
              <div key={availability.specialty} className="flex items-center justify-between p-3 border border-gray-200 rounded-lg">
                <div className="flex items-center gap-3">
                  <span className="text-xl">
                    {getSpecialtyIcon(availability.specialty)}
                  </span>
                  <div>
                    <p className="font-medium">{getSpecialtyName(availability.specialty)}</p>
                    <p className="text-sm text-gray-500">
                ...
                <Button
                  onClick={() => handle24hToggle(availability.specialty, !availability.emergencyMode)}
                  variant={availability.emergencyMode ? 'default' : 'outline'}
                  size="sm"
                  className={availability.emergencyMode ? 'bg-orange-600 hover:bg-orange-700' : ''}
                >
      ...
      <Card className="p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {availabilities.map((availability) => {
            ...
            return (
              <div key={availability.specialty} className="p-4 border border-gray-200 rounded-lg">
                ...
                <div className="space-y-2">
                  <div className="flex justify-between items-center text-sm">
                    ...
                    <span className="font-medium">
                      {availability.estimatedWaitTime === 999 ? 'N/A' : `${availability.estimatedWaitTime}min`}
                    </span>
                  </div>
                  
                  <div className="flex justify-between items-center text-sm">
                    <span className="text-gray-600">Próxima Disponibilidade:</span>
                    <span className="font-medium">
      ...
      <Card className="p-6 bg-blue-50 border-blue-200">
        <div className="flex items-start gap-3">
          ...
          <div className="text-sm text-blue-800">
            ...
            <p>
              O sistema monitora automaticamente a disponibilidade dos profissionais e ajusta 
              os tempos de espera em tempo real. Durante o modo emergência (24h), 
              as especialidades ficam disponíveis mesmo fora do horário comercial.
            </p>
          </div>
        </div>
      </Card>
