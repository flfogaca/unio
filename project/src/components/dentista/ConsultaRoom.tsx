...
export function ConsultaRoom({ consultaId }: ConsultaRoomProps) {
  ...
  useEffect(() => {
    const token = localStorage.getItem('token');
    console.log('🔌 Profissional: Tentando conectar WebSocket...', {
      user: user?.id,
    ...
    if (user && token) {
      ...
      console.log(
        '❌ Profissional: Não foi possível conectar - usuário ou token ausente'
      );
    }
  ...
  useEffect(() => {
    ...
    const handleConsultationFinished = (data: any) => {
      ...
      if (data.consultationId === consultaId) {
        ...
        if (data.redirectTo) {
          console.log(
            '🚀 Profissional: Redirecionando automaticamente para:',
            data.redirectTo
          );
          setTimeout(() => {
          ...
          console.log('ℹ️ Profissional: Sem URL de redirecionamento');
        }
      } else {
  ...
  const handleConfirmEnd = async () => {
    ...
    try {
      ...
      setTimeout(() => {
        console.log(
          '🚀 Profissional: Redirecionando automaticamente para dashboard'
        );
        const roleRoutes = {
          dentista: '/dentista',
          psicologo: '/dentista',
        ...
        const route =
          roleRoutes[user?.role as keyof typeof roleRoutes] || '/dentista';
