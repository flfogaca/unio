...
export function ConsultasAtivas() {
  ...
  const minhasConsultasAtivas = items.filter(
    item => item.status === 'em-atendimento' && item.dentistaId === user?.id
  );

  const consultasFinalizadasHoje = items.filter(
    item =>
  ...
  const proximaConsulta = items.find(
    item =>
      item.status === 'em-fila' &&
  ...
  const stats = [
    {
      ...
      change:
        minhasConsultasAtivas.length > 0 ? 'Em andamento' : 'Nenhuma ativa',
              ...
              {minhasConsultasAtivas.length > 0 ? (
                <div className='space-y-4'>
                  {minhasConsultasAtivas.map(consulta => (
                    <div
                      key={consulta.id}
                      className='border border-accent/20 bg-accent/5 p-4 rounded-lg'
                    >
                ...
                <div className='flex items-center justify-between'>
                  ...
                  <span className='text-sm text-gray-600'>
                    {consultasFinalizadasHoje.length +
                      minhasConsultasAtivas.length}
                  </span>
                </div>
                <div className='flex items-center justify-between'>
                  <span className='text-sm font-medium'>Tempo trabalhado:</span>
                  <span className='text-sm text-gray-600'>
                    {Math.round(tempoTotalHoje)}min
                  </span>
                </div>
                <div className='flex items-center justify-between'>
                ...
                <div className='flex justify-between'>
                  ...
                  <span className='text-sm font-medium'>
                    {consultasFinalizadasHoje.length > 0
                      ? Math.round(
                          tempoTotalHoje / consultasFinalizadasHoje.length
                        )
                      : 0}
                    min
                  </span>
