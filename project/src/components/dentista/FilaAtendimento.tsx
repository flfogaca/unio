...
export function FilaAtendimento() {
  ...
  const especialidadeProfissional = user?.role
    ? roleToSpecialtyMap[user.role]
    : null;
    ...
    .filter(
      item =>
        ...
        (!especialidadeProfissional ||
          item.especialidade === especialidadeProfissional)
    )
    .sort((a, b) => {
  ...
  const minhasConsultas = items.filter(
  ...
  const handleAssumeConsulta = async (itemId: string) => {
    if (user?.id) {
      console.log('🦷 Dentista assumindo consulta:', {
        itemId,
        userId: user.id,
      });
      try {
  ...
  const getTempoEspera = (criadoEm: Date) => {
    const diff = Math.floor(
      (currentTime.getTime() - criadoEm.getTime()) / (1000 * 60)
    );
