...
export function HistoricoConsultas() {
  ...
  const [consultaSelecionada, setConsultaSelecionada] = useState<string | null>(
    null
  );
  ...
  const consultasHistorico = items
    .filter(item => {
      console.log(
        `  - Checando item ${item.id}: pacienteId=${item.pacienteId} vs user=${user?.id}`
      );
    ...
    .map(item => {
      ...
      return {
        ...
        especialidade:
          especialidadeMap[item.especialidade] || item.especialidade,
        status: item.status,
        duracao,
  ...
  const especialidades = [
    ...new Set(consultasHistorico.map(c => c.especialidade)),
  ];
          ...
          <CardContent className='p-6'>
            <div className='flex items-center justify-between'>
              <div>
                ...
                <p className='text-2xl font-bold text-primaryDark'>
                  {consultasHistorico.reduce(
                    (acc, c) => acc + parseInt(c.duracao),
                    0
                  )}{' '}
                  min
                </p>
        ...
        <CardContent className='p-6'>
          <div className='flex flex-col lg:flex-row gap-4'>
            ...
            >
              ...
              Filtros
              <ChevronDown
                className={`h-4 w-4 ml-2 transition-transform ${showFiltros ? 'rotate-180' : ''}`}
              />
            </Button>
          </div>
                    ...
                    {consulta.proximaConsulta && (
                      <div className='bg-accent/5 p-3 rounded-lg mb-4'>
                        <p className='text-sm text-accent'>
                          ...
                          {(
                            consulta.proximaConsulta as any
